...
/*
	if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
		struct rte_device *dev = dev->device;
		for (unsigned port =0; port <RTE_MAX_ETHPORTS; port++)
			if (rte_eth_devices[port].device == dev)
				break;
		RTE_VERIFY(port != RTE_MAX_ETHPORTS);

		struct rte_eth_dev *dev =
			&rte_eth_devices[port];

		struct hn_nvs_process_priv *process_priv = dev->process_private;
		const struct rte_vmbus_device *secondary_dev =
			process_private->vmbus_dev;

		return vmbus_uio_irq_control(secondary_dev, 1);
	}
*/
...
void rte_vmbus_chan_signal_read(struct rte_vmbus_device *dev, struct vmbus_channel *chan, uint32_t bytes_read)
{
		      ...
		      struct vmbus_channel **new_chan)
{
	...
	chan->device = device;
	printf("%s: create channel %p device %p\n", __func__, chan, device);
